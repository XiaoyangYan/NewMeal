	...
	submitFormRegister = e => {
		...
		if (this.validator.allValid() && (this.state.registerPasswordAssure === this.state.registerPassword)) {
			...
			AjaxServiceSignForm.registerNewUser(Users).then(
				() => {
					AuthenticatedService.registerSuccessfulLogin(this.state.email, this.state.registerPassword);
...
// 		console.log(message);
// 		if (message.data === "valid"){
// 			AuthenticatedService.registerSuccessfulLogin(this.state.email, this.state.password);
